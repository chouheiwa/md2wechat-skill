...
本项目使用 Claude AI 生成精美的微信公众号 HTML 排版。所有主题都通过 AI 生成带内联 CSS 的 HTML。
...
#### 使用方法
```bash
md2wechat convert article.md --theme custom --custom-prompt "你的自定义提示词"
```
...
```bash
md2wechat convert story.md --theme autumn-warm --preview
```
...
```bash
md2wechat convert travel.md --theme spring-fresh --preview
```
...
```bash
md2wechat convert tech-post.md --theme ocean-calm --preview
```
